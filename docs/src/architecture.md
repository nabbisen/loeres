...
|---|---|---|
| `loeres-core` | `#![no_std]`, no `alloc` | Mathematical contracts: scalar capabilities, vector/matrix access, problem families, solver outcome/status, dimensions, allocation-free errors. Defines no storage. |
...
```text
loeres-cluster ─▶ loeres-backend-std ─▶ loeres-core ◀─ loeres-backend-static ◀─ loeres-device
       └────────────────────────────────▶ loeres-core ◀───────────────────────────────┘
```
...
`loeres-core` depending on any backend or execution crate; any edge-facing
...
- **Storage-agnostic access.** `loeres_core::access` defines shape, indexing,
  borrowing, and fallible access without committing to a memory layout or
  implying heavy linear-algebra kernels.
- **Solver outcome / status taxonomy.** `loeres_core::solver` owns the single
