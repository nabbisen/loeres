...
2. `check-rfcs`: assert no file under `rfcs/done/` contains a `## 0.N Amendment` / `### 0.N Amendment` heading. Fail-closed. Two tests (a `done/` file with such a heading fails; `accepted/` with one passes).
