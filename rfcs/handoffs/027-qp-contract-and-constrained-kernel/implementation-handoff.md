...
**Review 056's L1 is settled as option (a) by Amendment 5 §0.5.4** - once the status is truthful a batch caller reading `NotConverged` is correctly warned, so `BatchItemOutcome` does **not** need widening and no new RFC is required: document the batch path as carrying status only, and point callers needing the magnitudes at the typed entrypoint `solve_constrained_projected_first_order_dyn`. Also state that `Converged` means feasible within `projection_tolerance`, and that `NotConverged` with `NoProgress` on a constrained solve indicates an infeasible or too-tightly-capped polyhedron (C4), evaluated at the final iteration under `ConstantIteration` (RFC 029).

Then: requirements §5.1.3 PF disposition (PF-002 implemented; PF-001 contract-only, not solved; PF-003 unchanged); external design §3.2 (trait satisfies the category; builder deferred); crate READMEs; user guides. State §11.6's limitations in every user-facing surface, verbatim in substance.
...
### 3.2 Post-S4 corrective items (architect review 057) — CLOSED by `073ae70`, accepted in review 058

*C1-C3 are complete and verified. Retained as the record of what was required.
Review 058 additionally opened **proposed RFC 029** from the implementer's §5.3
observation: under `ConstantIteration` both device kernels report `Converged`
from a sticky flag, so a divergent run returns `Converged` while still moving.
That is a separate, pre-existing defect shared with RFC 006, owner-scheduled,
