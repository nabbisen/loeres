...
| [001](done/001-stratified-scalar.md) | Stratified Scalar Capability Model | Implemented (v0.6.0) | `loeres-core` `scalar` module; six tiers `BaseScalar` … `AdvancedNumericalScalar`, with `f32`/`f64` baseline impls. |
...
4. `xtask zero-bleed` must reject any transitive `std` or `alloc` edge into `loeres-core`, `loeres-backend-static`, or `loeres-device` baseline builds.
