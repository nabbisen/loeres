...
constraint, cyclic sweeps, monotone convergence to the projection. Box bounds are
handled as the closed-form `clamp` inside each sweep, so the box path degenerates
to RFC 006's kernel exactly when `m = 0` — **RFC 006/016 behaviour is preserved
bit-for-bit for `m = 0`**, and the conformance corpus asserts it.
...
error: the iterate is feasible-approximate, the outer loop continues, and the
report records `projection_cap_hits`.

### 11.4 Workspace (device)

`ConstrainedProjectedWorkspace<S, N, M>`: gradient scratch `N`, projection
scratch `N`, multipliers `M`, precomputed `‖aᵢ‖²` `M` — footprint
`(2N + 2M)·size_of::<S>() + header`, reported via `WorkspaceFootprint`,
`reset_for_entry` overwrite-on-use (RFC 005 always-reusable).

### 11.5 Validation (RFC 012 discipline)

Structural, always: shapes; `lo ≤ hi` finite; every `‖aᵢ‖² > 0` finite (a zero
row is `InvalidInput`); `step_scale` finite `> 0`; caps `≥ 1`. Policy-governed
