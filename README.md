...
> **v0.13.0 — Milestone 3: cluster orchestration infrastructure.** RFC 008 populates the `loeres-cluster` orchestration foundation (`batch` / `runtime` / `solve`): the per-item batch contract (`BatchSolveReport`, `BatchItemOutcome`, an explicit-count `BatchSummary`, `ClusterSolution`), a runtime-agnostic configuration / cancellation / executor layer (`ClusterSolveConfig`, a cluster-owned `ClusterCancellationToken` over `Arc<AtomicBool>`, with `parallel-rayon` and `async-tokio` backends behind feature gates so no runtime type appears in the baseline public surface), and the `ClusterJob` hybrid-dispatch seam. It consumes the RFC 012 validation vocabulary through `ClusterValidationPolicy`. The status/error split is preserved at the batch layer: a bounded-terminus non-convergence is a *solved* item carrying `SolveStatus::NotConverged`, never a failure, and a contained worker panic surfaces as `Panicked` (only under `panic = "unwind"`). This release is cluster orchestration **infrastructure, not a production numerical cluster solver**: no std-side solver kernel exists yet — `ClusterJob` is the stable seam where one plugs in later, and the machinery is validated against deterministic test jobs (orchestration behavior, not numerical correctness). The std-side kernel and trusted-pipeline / caching are deferred to a follow-on cluster RFC; the size-budget gate to RFC 010. Earlier in Milestone 3, v0.12.x landed the RFC 012 validation-state vocabulary and v0.11.x the dynamic dense/sparse storage adapters (RFC 007). Edge crates remain `no_std` / no-`alloc`, verified on a bare-metal target; the cluster's async/parallel backends never bleed into them.
...
- RFCs: [`rfcs/`](rfcs/) — Milestone 1–3 and cross-cutting contracts. Implemented contracts live in [`rfcs/done/`](rfcs/done/) (the lifecycle policy `000`, plus `001`/`002`/`003`/`004`/`005`/`006`/`007`/`008`/`012`/`014`); the rest are under [`rfcs/proposed/`](rfcs/proposed/). See the [RFC index](rfcs/README.md).
