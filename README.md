...
> **v0.14.0 — Milestone 3: first std-side cluster kernel.** RFC 016 lands the first production std-side numerical kernel in `loeres-cluster`: a dynamic box/bound-constrained **projected first-order** solver over `loeres-backend-std` `DenseVector` — the dynamic-storage analog of the RFC 006 device kernel — exposed as a typed entrypoint (`solve_projected_first_order_dyn` over a `ClusterProjectedFirstOrderProblem` oracle and a reusable `ClusterProjectedFirstOrderWorkspace`) and an erased `ClusterProjectedFirstOrderJob` that plugs into the RFC 008 `ClusterJob` seam, graduating cluster orchestration from deterministic test jobs to real solving. Convergence is step-norm (`max_i |Δxᵢ| ≤ tolerance`), aligned with RFC 006; the status/error split holds — non-convergence at the iteration cap is a *solved* `NotConverged` item, never a failure. Validation follows RFC 012: structural checks always run, and the returned `ProjectedFirstOrderSolveRecord` carries a `checked_scope` plus a named `ProjectedFirstOrderFiniteEvidence` (`Scanned` / `Trusted(..)` / `DomainInapplicable`) so a `TrustedByCaller` finite-scan skip is recorded honestly — never as fabricated coverage — and never yields `Solved` over NaN/Inf (in-loop non-finite → `NumericalDomain`). Trusted-pipeline / caching is deferred to RFC 015. This builds on the RFC 008 orchestration foundation (v0.13.0) — the per-item batch contract, runtime-agnostic config / cancellation / executor layer, and `parallel-rayon` / `async-tokio` backends behind feature gates. Edge crates remain `no_std` / no-`alloc`, verified on a bare-metal target; the cluster's async/parallel backends and this std-side kernel never bleed into them.
