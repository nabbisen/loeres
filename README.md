...
> **v0.12.0 — Milestone 3: the core validation-state vocabulary.** RFC 012 adds `loeres::validation`, the allocation-free vocabulary describing *what input validation has been performed*: a `ValidationScope` coverage bitset, `FiniteCoverage` (`Checked` / `NotApplicable`), a `#[non_exhaustive]` `TrustKind`, a `TrustToken`, the `ValidationCoverage` recording descriptor, `TrustedByCaller` evidence, and the `ValidationState` category enum. It is deliberately **core-first**: RFC 012 runs no scans and changes no shipped solver signature — backends remain the validators (`validate_finite`, the device inline checks) and record their outcome in these types, while structural validity stays a construction precondition (RFC 004 / 007). Finite-not-applicable is kept distinct from a missing-capability *unavailable* (which is rejected, not validated). Cluster trusted-pipeline and caching are deferred to RFC 008, the shared conformance corpus to RFC 013. Earlier in Milestone 3, v0.11.0 landed the dynamic dense/sparse storage adapters (RFC 007), hardened in v0.11.1. Core types are `no_std` / no-`alloc`, verified on a bare-metal target. v0.12.1 made `ValidationCoverage` coherent by construction — private fields, a scope normalized to always include `FINITE`, and accessors — so its scope bit and `finite` field cannot contradict before RFC 008 consumes them.
...
- RFCs: [`rfcs/`](rfcs/) — Milestone 1–3 and cross-cutting contracts. Implemented contracts live in [`rfcs/done/`](rfcs/done/) (the lifecycle policy `000`, plus `001`/`002`/`003`/`004`/`005`/`006`/`007`/`012`/`014`); the rest are under [`rfcs/proposed/`](rfcs/proposed/). See the [RFC index](rfcs/README.md).
