...
**RFC 010** is now **implemented (v0.16.0)**: `xtask` has a stable verification command namespace and `cargo xtask check` is the canonical aggregate release gate (`release-gate` remains an alias). The aggregate runs host checking, zero-bleed, no-std, feature matrix, target profiles, RFC lifecycle/link validation, public API scanning, panic audit, size-budget reporting, unsafe audit, conformance hook, and repository link audit. RFC 013 still owns concrete conformance fixtures, so the conformance gate reports an explicit pass-through until a corpus exists. Runtime crate APIs are unchanged from v0.15.0.
...
7. RFC 010 — xtask verification governance. **Resolved — implemented (v0.16.0); `cargo xtask check` is the aggregate gate, with target/profile/API/size/unsafe/link/conformance hooks in place.**
