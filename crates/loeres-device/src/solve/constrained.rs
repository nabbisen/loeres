    ...
    S: FiniteScalar + MetricScalar + DivisibleScalar,
{
    ...
    for sweep in 0..config.projection_max_sweeps {
        ...
        if sweep == midpoint_sweep {
            snap.midpoint_multiplier = largest_multiplier(workspace.multipliers.as_slice());
        }
        if sweep == final_sweep {
            snap.final_multiplier = largest_multiplier(workspace.multipliers.as_slice());
        }
        ...
        if sweep == midpoint_sweep {
            snap.midpoint_violation = max_constraint_violation(problem, x)?;
        }
        if sweep == final_sweep {
            snap.final_violation = max_constraint_violation(problem, x)?;
        }
